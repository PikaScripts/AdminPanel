...
const editmodal = (element) =>{
  ...
  `
  editlic.addEventListener('input', updatePrice);
  edittrack.addEventListener('input', updatePrice);
}
...
const edit = () =>{
  ...
  const projectCount = parseInt(inputlicensevalue.value);
  const tariffCount = parseInt(inputtrackvalue.value);
